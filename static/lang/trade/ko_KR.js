export const trade = {
  // #####################币币交易-买卖单#####################''통화 거래 - 판매 주문'
  'trade_coin_buying_and_sell': '구매 주문서',
  'trade_coin_entrusted_price': '수수료 가격',
  'trade_coin_entrusted_amount': '커미션 금액',
  // #####################查看更多委单记录#####################''더 많은 주문 기록보기'
  'trade_coin_commissioned_current': '현재위원회',
  'trade_coin_entrust': '위원회',
  'trade_coin_entrust_money1': '위탁 금액',
  'trade_coin_text1': '아직 로그인하지 않았습니다. 제발',
  'trade_coin_text2': '또는',
  'trade_coin_text3': '보기 후에',
  'trade_coin_revocation': '취소',
  'trade_coin_history': '역사',
  'trade_coin_history_entrust1': '역사위원회',
  // #####################限价交易、市价交易#####################''제한 거래, 시장 가격 거래'
  'trade_exchange_currency_data': '통화 정보',
  'trade_exchange_price_deal': '제한 거래',
  'trade_exchange_can_buy': '살 수있다.',
  'trade_exchange_vendibility': '판매 가능',
  'trade_exchange_estimated_turnover': '예상 거래 금액',
  'trade_exchange_market': '시장 거래',
  'trade_exchange_market_price': '시장 거래 가격',
  'trade_exchange_after_operation': '후 작업',
  'trade_exchange_currency_available': '사용 가능한 통화가 충분하지 않습니다.',
  // 全球行情''세계 시장'
  'trade_global_market': '세계 시장',
  // K线''K 라인'
  'trade_time_share': '시분할',
  'trade_latest_price': '최신 가격',
  'trade_ups_and_downs': '흥망 성쇠',
  'trade_highest': '최고',
  'trade_minimum': '최소',
  // 最近成交记录''최근 거래 내역'
  'trade_order_recent_deal': '최근 거래',
  // #####################币币交易 行情#####################''통화 거래'
  'trade_market_bazaar': '시장',
  'trade_market_amount_increase': '이득',
  'trade_market_optional': '선택 사항'
}
